function [x,iter,diff] = SOR_diff(A,b,x0,omega,tol,itmax)
%% Metodo iterativo de Sobrerelajacion sucesiva (SOR) - VERSION MODIFICADA
% Criterio de parada: ||x^(k) - x^(k-1)|| < tol
% (mismo criterio que Jacobi y GaussSeidel)
%
...
% diff   : diferencia ||x^(k) - x^(k-1)||
%
% Autor: Luciano Ponzellini Marinelli (2025)
% Modificacion Ejercicio 7a: Agustin Formenti (2025)
...
for k = 1:itmax
    ...
    % Criterio de parada: mismo que Jacobi y Gauss-Seidel
    diff = normap(x-x0,2);
